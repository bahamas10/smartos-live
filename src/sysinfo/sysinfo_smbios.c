...
static void do_system_common(smbios_info_t *info, nvlist_t *nvl) {
	if (info->smbi_manufacturer[0] != '\0')
		nvlist_add_string(nvl, "Manufacturer", info->smbi_manufacturer);
	if (info->smbi_product[0] != '\0')
		nvlist_add_string(nvl, "Product", info->smbi_product);
	if (info->smbi_version[0] != '\0')
		nvlist_add_string(nvl, "HW Version", info->smbi_version);
	if (info->smbi_serial[0] != '\0')
		nvlist_add_string(nvl, "Serial Number", info->smbi_serial);
	if (info->smbi_asset[0] != '\0')
		nvlist_add_string(nvl, "Asset Tag", info->smbi_asset);
	if (info->smbi_location[0] != '\0')
		nvlist_add_string(nvl, "Location Tag", info->smbi_location);
	if (info->smbi_part[0] != '\0')
		nvlist_add_string(nvl, "Part Number", info->smbi_part);
}
...
static void do_system(smbios_system_t *s, nvlist_t *nvl) {
	...
	nvlist_add_string(nvl, "UUID", uuid);
	nvlist_add_string(nvl, "SKU Number", s->smbs_sku);
	nvlist_add_string(nvl, "HW Family", s->smbs_family);
}
...
static void do_processor_common(smbios_info_t *info, nvlist_t *nvl) {
	if (info->smbi_version[0] != '\0') {
		...
		nvlist_add_string(nvl, "CPU Type", type);
	}
}
...
static void do_processor(smbios_processor_t *p, nvlist_t *nvl) {
	if (p->smbp_corecount != 0) {
		if (p->smbp_corecount != 0xff || p->smbp_corecount2 == 0)
			nvlist_add_int32(nvl, "CPU Total Cores", p->smbp_corecount);
		else
			nvlist_add_int32(nvl, "CPU Total Cores", p->smbp_corecount2);
	} else {
		// TODO warning
	}
}
