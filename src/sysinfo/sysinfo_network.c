...
phys_macaddr(void *arg, dladm_macaddr_attr_t *attr)
{
	...
	nvlist_add_string(int_nvl, "MAC Address", macstr);
	...
	nvlist_add_string_array(int_nvl, "NIC Names", tags, i);
...
get_link_state(char *link, nvlist_t *nvl)
{
	...
	nvlist_add_string(nvl, "Link Status", linkstate_str);
done:
	kstat_close(kcp);
}
...
sysinfo_network(nvlist_t *root_nvl)
{
	...
	if (nvlist_alloc(&net_nvl, NV_UNIQUE_NAME, 0) != 0) {
		warn("nvlist_alloc");
		goto done;
	}
	...
	for (ifa = ifap; ifa; ifa = ifa->ifa_next) {
		if (ifa->ifa_addr->sa_family == AF_INET) {
			...
			if (nvlist_lookup_nvlist(net_nvl, link, &int_nvl) == 0)
				nvlist_add_string(int_nvl, "ip4addr", addr);
		}
	}
	freeifaddrs(ifap);

	nvlist_add_nvlist(root_nvl, "Network Interfaces", net_nvl);
